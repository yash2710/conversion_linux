...
SYSCALL_DEFINE2(conversion_determ_init, unsigned long, address, unsigned long, token_addr)
{
  struct vm_area_struct *vma;

  vma = find_vma(current->mm, address);

  printk(KERN_EMERG "in determ init sys call, is snapshot? %d %p address %p\n", 
  ...
  if (vma && mmap_snapshot_instance.is_snapshot &&
    ...
    mmap_snapshot_instance.conversion_determ_init(vma, token_addr);   //TODO: this function name in the struct should change$
  }
}
