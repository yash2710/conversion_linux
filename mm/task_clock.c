...
SYSCALL_DEFINE2(task_clock_do, uint32_t, operation, uint64_t, value)
{
	switch (operation){
		case TASK_CLOCK_OP_STOP:
                	if (current->task_clock.user_status && task_clock_func.task_clock_entry_stop){
	                  task_clock_func.task_clock_entry_stop(current->task_clock.group_info);
        	        }
			break;
		case TASK_CLOCK_OP_START:
			if (current->task_clock.user_status && task_clock_func.task_clock_entry_start){
	                  task_clock_func.task_clock_entry_start(current->task_clock.group_info);
        	        }
			break;
		case TASK_CLOCK_OP_START_COARSENED:
                	if (current->task_clock.user_status && task_clock_func.task_clock_entry_start_no_notify){
                  	  task_clock_func.task_clock_entry_start_no_notify(current->task_clock.group_info);
                	}
			break;
	}
}
